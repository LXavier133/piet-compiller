...
vector<vector<unsigned long>> img;
...
int main(){__
	...
	if((height <=0 || width <= 0) && !error){cout<<"the image is too big!"; error=true;}
	img.resize(width, vector<unsigned long>(height));
	for(long long j=0; j<height;j++) for (long long i=0; i<width;i++){
		int r,g,b;cin>>r>>g>>b;
		img[i][j]=rgb(r,g,b);
		if(color.find(img[i][j])==color.end() && !error){cout<<"piet does not recognize the color "<< hex << img[i][j] << dec <<" at x= "<< i<< " y= " << j<<endl;error=true;}}
return 0;}
